...
ddMiddleware :: (Typeable (Route site), ToTypedContent res) => HandlerT site IO res -> HandlerT site IO res
ddMiddleware m = do
  ...
  liftIO $ case L.lookup statsTagsKey $ vault req of
    Just ref -> case mRoute of
      ...
      Just r -> modifyIORef ref (tag "route_name" (pack $ tyConName $ typeRepTyCon $ typeOf r) :)
    Nothing -> return ()
  m
